...
from model.modules.sparse_transformer import SoftSplit, SoftComp
from model.nvtx import nvtx as _nvtx
import os as _os, time as _time
# Opt-in CUDA-synced wall-clock accumulator for ONLY the transformer block.
# Enabled with PROPAINTER_TIME_TST=1; read/reset via the module globals below.
_TIME_TST = _os.environ.get('PROPAINTER_TIME_TST', '0') == '1'
TST_TIME_MS = 0.0
TST_CALLS = 0
ENC_TIME_MS = 0.0
# Optional: run the encoder conv stack in channels_last (NHWC). On sm_120
# (RTX 5090) at ProPainter's shapes this MEASURED SLOWER (+19-37% encoder): the
# grouped convs + per-concat layout conversions outweigh any tensor-core gain,
# and cuDNN's NCHW fp16 path is already fast here. Kept gated, default OFF;
# PROPAINTER_CHANNELS_LAST=1 to re-test on other shapes/hardware.
_CHANNELS_LAST = _os.environ.get('PROPAINTER_CHANNELS_LAST', '0') == '1'
# Accelerated CUDA drop-in for the transformer block; falls back to the original
# PyTorch path automatically (or with PROPAINTER_FAST=0). Same checkpoint loads.
from model.propainter_fast import FastTemporalSparseTransformerBlock as TemporalSparseTransformerBlock
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        bt, c, _, _ = x.size()
        # h, w = h//4, w//4
        # channels_last keeps the heavy fp16 convs on tensor cores; reshape (not
        # view) tolerates the NHWC strides, and the group-interleave is a logical
        # op so it is layout-invariant. Return contiguous (NCHW) so the caller's
        # downstream .view() still works.
        out = x.to(memory_format=torch.channels_last) if _CHANNELS_LAST else x
        for i, layer in enumerate(self.layers):
            ...
            if i > 8 and i % 2 == 0:
                g = self.group[(i - 8) // 2]
                xg = x0.reshape(bt, g, -1, h, w)
                og = out.reshape(bt, g, -1, h, w)
                out = torch.cat([xg, og], 2).reshape(bt, -1, h, w)
                if _CHANNELS_LAST:
                    out = out.to(memory_format=torch.channels_last)
            out = layer(out)
        return out.contiguous() if _CHANNELS_LAST else out
...
class InpaintGenerator(BaseNetwork):
    ...
    def forward(self, masked_frames, completed_flows, masks_in, masks_updated, num_local_frames, interpolation='bilinear', t_dilation=2):
        ...
        with _nvtx('model/encoder'):
            ...
            if _TIME_TST:
                torch.cuda.synchronize()
                ENC_TIME_MS += (_time.perf_counter() - _te) * 1000.0
