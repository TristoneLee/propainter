...
from model.nvtx import nvtx as _rf
...
class FusionFeedForward(nn.Module):
    def __init__(self, dim, hidden_dim=1960, t2t_params=None):
        ...
        # normalizer is data-independent (only depends on output_size + dtype +
        # device). Cache it across forwards to skip the repeat allocation +
        # col2im that otherwise runs once per inner block × per inference iter.
        self._normalizer_cache: dict = {}

    def _get_normalizer(self, output_size, n_vecs, dtype, device):
        key = (tuple(output_size), dtype, device)
        cached = self._normalizer_cache.get(key)
        if cached is not None:
            return cached
        # Build a single [1, kernel_shape, n_vecs] of ones and fold it once.
        # The folded result has shape [1, 1, oh, ow] and broadcasts at the
        # divide step against [BT, c, oh, ow], so no batch replication needed.
        ones = torch.ones(1, self.kernel_shape, n_vecs, dtype=dtype, device=device)
        normalizer = F.fold(ones,
                            output_size=output_size,
    ...
    def forward(self, x, output_size):
        ...
        with _rf("tst/ffn/fc1"):
            x = self.fc1(x)
        b, n, c = x.size()
        with _rf("tst/ffn/fold_unfold"):
            normalizer = self._get_normalizer(output_size, n_vecs, x.dtype, x.device)
...
class SparseWindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, T_ind=None, attn_mask=None):
        ...
        with _rf("tst/attn/qkv_proj"):
            q = self.query(x)
            k = self.key(x)
            v = self.value(x)
        with _rf("tst/attn/win_partition_qkv"):
        ...
        with _rf("tst/attn/unshuffle_proj"):
