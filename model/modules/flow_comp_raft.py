...
def initialize_RAFT(model_path='weights/raft-things.pth', device='cuda', backbone='sea_raft'):
    """Initialize the optical-flow backbone.

    backbone='sea_raft' (default): SEA-RAFT small; self-loads its Tartan weights.
    backbone='raft'             : original RAFT (raft-things.pth), kept selectable
        as a latency baseline. SEA-RAFT remains the default backbone.
    """
    if backbone == 'raft':
        ...
        # raft-things.pth was saved from a DataParallel model ('module.' prefix).
    ...
    args = argparse.ArgumentParser()
    args.size = "small"
    model = torch.nn.DataParallel(SEA_RAFT(args))
...
class RAFT_bi(nn.Module):
    """Bidirectional optical flow (SEA-RAFT by default; RAFT selectable as baseline).

    Backbone is chosen by the `backbone` arg or env PROPAINTER_FLOW_BACKBONE
    (raft|sea_raft); default sea_raft so SEA-RAFT stays the production backbone.
    """
    def __init__(self, model_path='weights/raft-things.pth', device='cuda', backbone=None):
        super().__init__()
        self.backbone = backbone or os.environ.get('PROPAINTER_FLOW_BACKBONE', 'sea_raft')
        self.fix_raft = initialize_RAFT(model_path, device=device, backbone=self.backbone)
    ...
    def forward(self, gt_local_frames, iters=20):
        ...
        with torch.no_grad():
            ...
            if self.backbone == 'raft':
                # Original RAFT has no forward_bi: batch the forward and backward
                # pairs (same trick SEA-RAFT.forward_bi uses), take flow_up from
                # RAFT's (flow_low, flow_up) tuple. RAFT's all-pairs correlation
                # volume is ~O((HW/64)^2) memory per pair, so process in bounded
                # sub-batches (PROPAINTER_RAFT_BATCH, default 4) to avoid OOM at HD.
                # Original RAFT is fp32-only (it casts feature maps to .float()
                # internally, which clashes with the pipeline's --fp16 .half()).
                # Run it in fp32 and cast the flow back to the pipeline dtype.
                self.fix_raft.float()
                out_dtype = gt_local_frames.dtype
                img1 = torch.cat([gtlf_1, gtlf_2], dim=0).float()
                img2 = torch.cat([gtlf_2, gtlf_1], dim=0).float()
                bs = int(os.environ.get('PROPAINTER_RAFT_BATCH', '4'))
                ups = []
                for i in range(0, img1.shape[0], bs):
                    _, fu = self.fix_raft(img1[i:i + bs], img2[i:i + bs],
                                          iters=iters, test_mode=True)
                    ups.append(fu)
                flow_up = torch.cat(ups, dim=0).to(out_dtype)
                n = gtlf_1.shape[0]
                gt_flows_forward, gt_flows_backward = flow_up[:n], flow_up[n:]
            else:
                gt_flows_forward, gt_flows_backward = self.fix_raft.forward_bi(gtlf_1, gtlf_2, iters=iters, test_mode=True)
