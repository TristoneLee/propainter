...
def bilinear_sampler(img, coords, mode='bilinear', mask=False):
    ...
    # Ensure grid has the same dtype as img for fp16 compatibility
    grid = grid.to(dtype=img.dtype)
...
def coords_grid(batch, ht, wd, device, dtype=None):
    coords = torch.meshgrid(torch.arange(ht, device=device), torch.arange(wd, device=device))
    coords = torch.stack(coords[::-1], dim=0).float()
    coords = coords[None].repeat(batch, 1, 1, 1)
    # Convert to specified dtype if provided (for fp16 compatibility)
