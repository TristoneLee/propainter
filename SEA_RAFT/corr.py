...
class CorrBlock:
    ...
    def __call__(self, coords, dilation=None):
        ...
        if dilation is None:
            dilation = torch.ones(batch, 1, h1, w1, device=coords.device, dtype=coords.dtype)
        ...
        # Keep the original dtype instead of forcing float
        return out
    ...
    def corr(fmap1, fmap2, num_head):
        ...
        # Use tensor with same dtype for division
        return corr / torch.sqrt(torch.tensor(dim, dtype=corr.dtype, device=corr.device))
