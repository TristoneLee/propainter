...
def main_worker(args):
    ...
    print(f'Using {num_gpus} GPU(s) for evaluation')
    ...
    if num_gpus > 1:
        fix_flow_complete = torch.nn.DataParallel(fix_flow_complete)
    ...
    for index, items in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # ---- complete flow ----
            flow_complete_module = fix_flow_complete.module if num_gpus > 1 else fix_flow_complete
            pred_flows_bi, _ = flow_complete_module.forward_bidirect_flow(gt_flows_bi, masks)
            pred_flows_bi = flow_complete_module.combine_flow(gt_flows_bi, pred_flows_bi, masks)

            # ---- temporal propagation ----
            model_module = model.module if num_gpus > 1 else model
            prop_imgs, updated_local_masks = model_module.img_propagation(masked_frames, pred_flows_bi, masks, 'nearest')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_ids', type=str, default=None, help='GPU ids to use, e.g., "0,1,2,3". If not specified, all available GPUs will be used.')
